...
public class RectangleParticle extends AbstractParticle {
	 ...
	 *            <p>
	 *            Note that RectangleParticles can be fixed but still have their
	 *            rotation property changed.
	 *            </p>
	 */
	// TG TODO check defaults
	public RectangleParticle(double x, double y, double width, double height,
			double rotation, boolean fixed, double mass, double elasticity,
			double friction) {
		...
		// TG TODO I have added these lines as in actionscript3 these are called
		// when initialized
	...
	/**
	 * The rotation of the RectangleParticle in radians. For drawing methods you
	 * may want to use the <code>angle</code> property which gives the rotation
	 * in degrees from 0 to 360.
	 * 
	 * <p>
	 * Note that while the RectangleParticle can be rotated, it does not have
	 * angular velocity. In otherwords, during collisions, the rotation is not
	 * altered, and the energy of the rotation is not applied to other colliding
	 * particles. A true rigid body is planned for a later release.
	 * </p>
	 */
	...
	/**
	 * An Array of 4 contact particles at the corners of the RectangleParticle.
	 * You can attach other particles or constraints to these particles. Note
	 * this is a one-way effect, meaning the RectangleParticle's motion will
	 * move objects attached to the corner particles, but the reverse is not
	 * true.
	 * 
	 * <p>
	 * In order to access one of the 4 corner particles, you can use array
	 * notation e.g., <code>myRectangleParticle.cornerParticles[0]</code>
	 * </p>
	 */
	public ArrayList getCornerParticles() {
		...
		if (_cornerParticles.size() == 0) {
			// TG TODO default ??
			CircleParticle cp1 = new CircleParticle(0.0, 0.0, 1.0, false, 1.0,
					0.3, 0.0);
			cp1.setCollidable(false);
			cp1.setVisible(false);
			APEngine.addParticle(cp1);

			CircleParticle cp2 = new CircleParticle(0.0, 0.0, 1.0, false, 1.0,
					0.3, 0.0);
			cp2.setCollidable(false);
			cp2.setVisible(false);
			APEngine.addParticle(cp2);

			CircleParticle cp3 = new CircleParticle(0.0, 0.0, 1.0, false, 1.0,
					0.3, 0.0);
			cp3.setCollidable(false);
			cp3.setVisible(false);
			APEngine.addParticle(cp3);

			CircleParticle cp4 = new CircleParticle(0.0, 0.0, 1.0, false, 1.0,
					0.3, 0.0);
	 ...
	 * corners of this RectangleParticle. This method would usually be called in
	 * a painting method if the locations of the corners were needed. If the
	 ...
	 * that you should always write your own particle classes that extend one of
	 * the APE particle classes. Then within that class you can define your own
	...
	public Interval getProjection(Vector axis) {
		...
		double radius = ((Double) _extents.get(0)).doubleValue()
				* Math.abs((double) axis.dot((Vector) _axes.get(0)))
				+ ((Double) _extents.get(1)).doubleValue()
				* Math.abs((double) axis.dot((Vector) _axes.get(1)));
	...
	public void updateCornerPositions() {
		...
		double ae0_x = ((Vector) _axes.get(0)).x
				* ((Double) _extents.get(0)).doubleValue();
		double ae0_y = ((Vector) _axes.get(0)).y
				* ((Double) _extents.get(0)).doubleValue();
		double ae1_x = ((Vector) _axes.get(1)).x
				* ((Double) _extents.get(1)).doubleValue();
		double ae1_y = ((Vector) _axes.get(1)).y
				* ((Double) _extents.get(1)).doubleValue();
	...
	private void updateCornerParticles() {
		for (int i = 0; i < 4; i++) {
			((AbstractParticle) getCornerParticles().get(i))
					.setpx(((Vector) _cornerPositions.get(i)).x);
			((AbstractParticle) getCornerParticles().get(i))
					.setpy(((Vector) _cornerPositions.get(i)).y);
		}
	}
