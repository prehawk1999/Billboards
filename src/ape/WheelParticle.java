...
public class WheelParticle extends CircleParticle {
	 ...
	 *            <p>
	 *            Note that WheelParticles can be fixed but still have their
	 *            rotation property changed.
	 *            </p>
	 */
	public WheelParticle(double x, double y, double radius, boolean fixed,
			double mass, double elasticity, double friction, double traction) {
	...
	/**
	 * The angular velocity of the WheelParticle. You can alter this value to
	 * make the WheelParticle spin.
	 */
	...
	/**
	 * The amount of traction during a collision. This property controls how
	 * much traction is applied when the WheelParticle is in contact with
	 * another particle. If the value is set to 0, there will be no traction and
	 * the WheelParticle will behave as if the surface was totally slippery,
	 * like ice. Acceptable values are between 0 and 1.
	 * 
	 * <p>
	 * Note that the friction property behaves differently than traction. If the
	 * surface friction is set high during a collision, the WheelParticle will
	 * move slowly as if the surface was covered in glue.
	 * </p>
	 */
	...
	/**
	 * An Array of 4 contact particles on the rim of the wheel. The edge
	 * particles are positioned relatively at 12, 3, 6, and 9 o'clock positions.
	 * You can attach other particles or constraints to these particles. Note
	 * this is a one-way effect, meaning the WheelParticle's motion will move
	 * objects attached to the edge particles, but the reverse is not true.
	 * 
	 * <p>
	 * In order to access one of the 4 edge particles, you can use array
	 * notation e.g., <code>myWheelParticle.edgeParticles[0]</code>
	 * </p>
	 */
	 ...
	 * edge positions of this WheelParticle. The edge positions are located
	 * relatively at the 12, 3, 6, and 9 o'clock positions.
	 */
	 ...
	 * that you should always write your own classes that either extend one of
	 * the APE particle and constraint classes, or is a composite of them. Then
	 * within that class you can define your own custom painting method.
	 */
	public void paint() {
	 ...
	 * @private
	 */
	public void resolveCollision(Vector mtd, Vector velocity, Vector normal,
			double depth, double order) {
	...
	private void resolve(Vector n) {
		...
		Vector combinedVelocity = getVelocity()
				.plusEquals(wheelSurfaceVelocity);
		...
		// some of the wheel's torque is removed and converted into linear
		// displacement
	...
	private void updateEdgeParticles() {
		for (int i = 0; i < 4; i++) {
			((CircleParticle) _edgeParticles.get(i))
					.setpx(((Vector) _edgePositions.get(i)).x);
			((CircleParticle) _edgeParticles.get(i))
					.setpy(((Vector) _edgePositions.get(i)).y);
		}
	}
