...
public final class CollisionDetector {
	...
	/**
	 * Tests the collision between two objects. If there is a collision it is
	 * passed off to the CollisionResolver class to resolve the collision.
	 */
	...
	/**
	 * Tests the collision between two RectangleParticles (aka OBBs). If there
	 * is a collision it determines its axis and depth, and then passes it off
	 * to the CollisionResolver for handling.
	 */
	private static void testOBBvsOBB(RectangleParticle ra, RectangleParticle rb) {

		// TG TODO i had to init this however it could have an effect on the
		// functionality.
		...
		for (int i = 0; i < 2; i++) {
			...
			double depthA = testIntervals(ra.getProjection(axisA),
					rb.getProjection(axisA));
			if (depthA == 0)
				return;

			Vector axisB = (Vector) rb.getAxes().get(i);
			double depthB = testIntervals(ra.getProjection(axisB),
			...
			if (absA < Math.abs(collisionDepth)
					|| absB < Math.abs(collisionDepth)) {
				boolean altb = absA < absB;
				collisionNormal = altb ? axisA : axisB;
				collisionDepth = altb ? depthA : depthB;
			}
		}
		CollisionResolver.resolveParticleParticle(ra, rb, collisionNormal,
				collisionDepth);
	}

	/**
	 * Tests the collision between a RectangleParticle (aka an OBB) and a
	 * CircleParticle. If there is a collision it determines its axis and depth,
	 * and then passes it off to the CollisionResolver for handling.
	 */
	private static void testOBBvsCircle(RectangleParticle ra, CircleParticle ca) {
		...
		for (int i = 0; i < 2; i++) {
			...
			double depth = testIntervals(ra.getProjection(boxAxis),
		...
		if (Math.abs(((Double) depths.get(0)).doubleValue()) < r
				&& Math.abs(((Double) depths.get(1)).doubleValue()) < r) {
		...
		CollisionResolver.resolveParticleParticle(ra, ca, collisionNormal,
				collisionDepth);
	}

	/**
	 * Tests the collision between two CircleParticles. If there is a collision
	 * it determines its axis and depth, and then passes it off to the
	 * CollisionResolver for handling.
	 */
	private static void testCirclevsCircle(CircleParticle ca, CircleParticle cb) {
		...
		if (collisionDepth > 0) {
			collisionNormal.divEquals(mag);
			CollisionResolver.resolveParticleParticle(ca, cb, collisionNormal,
					collisionDepth);
		}
	}
	...
	private static Vector closestVertexOnOBB(Vector p, RectangleParticle r) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (dist >= 0)
				dist = ((Double) r.getExtents().get(i)).doubleValue();
			else if (dist < 0)
				dist = -((Double) r.getExtents().get(i)).doubleValue();
